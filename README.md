...
本项目是非官方项目，不隶属于、代表或获得 Microsoft 授权；Microsoft、Windows、Snipping Tool 等名称归其各自权利人所有。

本项目不重新分发或嵌入 Microsoft 的 `oneocr.dll`、`oneocr.onemodel`、`onnxruntime.dll` 等文件，只读取用户本机已安装 Windows 截图工具中的 OneOCR 运行时文件。默认扫描路径为 `C:\Program Files\WindowsApps\Microsoft.ScreenSketch_*\SnippingTool`。

使用者应自行确保使用行为符合适用法律法规、第三方软件许可和服务条款；请勿将本项目用于侵犯隐私、未授权采集/识别或其他违法用途。
